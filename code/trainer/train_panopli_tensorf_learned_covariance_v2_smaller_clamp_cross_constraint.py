...
from dataset import get_dataset, get_inconsistent_single_dataset, get_segment_dataset,get_inconsistent_single_dataset_two_frame
from model.loss.loss import TVLoss, get_semantic_weights, SCELoss, contrastive_loss, ae_loss
from model.radiance_field.tensoRF_learned_variance import TensorVMSplit
...
class TensoRFTrainer(pl.LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        if self.current_epoch >= self.config.instance_optimization_epoch:
            ...
            for img_idx in range(len(batch[1]['rays'])):
                ...
                ### single_frame loss term
                for frame_idx in range(2):
                    instance_features, points_xyz = self.forward_instance(batch[1]['rays'][img_idx][frame_idx], True)
                    # loss_instance_clustering_, regularization_term,loss_concentrate = self.calculate_instance_clustering_loss(
                    #     batch[1]['instances'][img_idx][frame_idx*size_single_frame: (frame_idx+1)*size_single_frame], instance_features, batch[1]['confidences'][img_idx][frame_idx*size_single_frame: (frame_idx+1)*size_single_frame],
                    #     points_xyz)
                    loss_instance_clustering_, regularization_term = self.calculate_instance_clustering_loss(
                        batch[1]['instances'][img_idx][frame_idx], instance_features, batch[1]['confidences'][img_idx][frame_idx],
                        points_xyz)

                    if frame_idx==0:
                        loss_instance_clustering = loss_instance_clustering + loss_instance_clustering_
                    all_instance_feature.append(instance_features)  
                ### multi_frame concenntrating
                loss_multi_view_constraint, mining_pos_ration,correct_ratio = self.multiview_constraint(all_instance_feature, batch[1]['instances'][0],batch[1]['confidences'][0])
                loss_instance_clustering += 0.01 * loss_multi_view_constraint
            
            all_instance_feature[0].retain_grad()
            # all_instance_feature[1].retain_grad()
            ...
            self.log("train/regularization_term", regularization_term, on_step=True, on_epoch=False, prog_bar=True, logger=True, sync_dist=True)
            # self.log("train/loss_concentrate", loss_concentrate, on_step=True, on_epoch=False, prog_bar=True, logger=True, sync_dist=True)
            # self.log("train/loss_multi_view_constraint", loss_multi_view_constraint, on_step=True, on_epoch=False, prog_bar=True, logger=True, sync_dist=True)
            self.log("train/mining_pos_ration", mining_pos_ration, on_step=True, on_epoch=False, prog_bar=True, logger=True, sync_dist=True)
            # self.log("train/correct_ratio", correct_ratio, on_step=True, on_epoch=False, prog_bar=True, logger=True, sync_dist=True)
    ...
    # def multiview_constraint(self, all_instance_feature,labels_gt, confidences, size_single_frame):
    #     all_fast_centorid = []
    #     for frame_idx, instance_features in enumerate(all_instance_feature):
    #     # instance_feature, next_instance_feature = all_instance_feature[0],all_instance_feature[1]
    ...
    #         # normalized_fast_features = torch.nn.functional.normalize(fast_features[:,:3], p=2, dim=1) #* torch.clamp(torch.norm(fast_features[:,:3], p=2, dim=1, keepdim=True), max=1)
    #         # normalized_fast_features = torch.cat([normalized_fast_features, fast_features[:,3:]], dim=1)
    #         # fast_features = normalized_fast_features
            
            
    #         # normalized_slow_features = torch.nn.functional.normalize(slow_features[:,:3], p=2, dim=1) #* torch.clamp(torch.norm(slow_features[:,:3], p=2, dim=1, keepdim=True), max=1)
    #         # normalized_slow_features = torch.cat([normalized_slow_features, slow_features[:,3:]], dim=1)
    #         # slow_features = normalized_slow_features
    #         # min_covariance = 0.01
    ...
    #         label_frame = labels_gt[frame_idx*size_single_frame: (frame_idx+1)*size_single_frame]
    #         ## compute centroids
    #         fast_centroids = []
    #         fast_labels = torch.unique(label_frame)
    #         # fast_labels, slow_labels = torch.unique(labels_gt[fast_mask]), torch.unique(labels_gt[slow_mask])
    #         for l in fast_labels:
    #             mask_ = label_frame==l #.unsqueeze(-1)
    #             fast_centroids.append(fast_projections[mask_].mean(dim=0))
    #         fast_centroids = torch.stack(fast_centroids)

    #         if len(fast_labels) == 0:        
    ...
    #         all_fast_centorid.append(fast_centroids)

    #     # print(all_fast_centorid[0].shape)
    #     # print(all_fast_centorid[1].shape)
    #     multiview_distance = self.calculate_cross_distribution_distance(all_fast_centorid[0],all_fast_centorid[1])
    #     multiview_distance = torch.max(multiview_distance,dim=-1)[0]
    #     # print(multiview_distance.shape)
    #     # exit()
    #     # confidences_cross = confidence[0:size_single_frame].s
    #     mask = multiview_distance >0.99
    #     # print("detect ratio:", mask.sum()/((multiview_distance >0.0).sum()))
    #     mining_pos_ration = mask.sum()/((multiview_distance >0.0).sum())
    #     if mask.sum()==0:
    #         return torch.tensor(0.0, device=instance_features.device),torch.tensor(0.0, device=instance_features.device)
    #     multiview_distance = multiview_distance[mask]
    #     loss_multiview_concentrate = -1.0 * (multiview_distance).mean()
    ...
    #     return loss_multiview_concentrate,mining_pos_ration
            
        # print("Unknown instance loss mode")
        # raise NotImplementedError
    def multiview_constraint(self, all_instance_feature,labels_gt, confidences):
        all_fast_centorid = []
        for frame_idx, instance_features in enumerate(all_instance_feature):
        # instance_feature, next_instance_feature = all_instance_feature[0],all_instance_feature[1]
            fast_features, slow_features = instance_features.split(
            ...
            # 
            label_frame = labels_gt[frame_idx]
            # confidence_frame = labels_gt[frame_idx]
            # label_frame_2 = instance_gt[1].unsqueeze(0)
            fast_centroids = []
            # fast
            fast_labels = torch.unique(label_frame)
        ...
        mask = multiview_distance > 0.9 # and  position, DINO_v2, tracking
        ...
        if mask.sum()==0:
            # return torch.tensor(0.0, device=instance_features.device)
            return torch.tensor(0.0, device=instance_features.device),torch.tensor(0.0, device=instance_features.device), torch.tensor(0.0, device=instance_features.device)
        ...
        return loss_multiview_concentrate, 0.0,0.0
            
        print("Unknown instance loss mode")
        raise NotImplementedError


    def calculate_distribution_distance(self, distribution_1, distribution_2):
        # centroid_mean_ = centroid_[:,:3]
        # centroid_covariance = centroid_[:,3:]
        # make sure the output is positive
        # centroid_covariance[...,3:] =  F.elu(centroid_covariance[...,3:]) + 1.0 + min_covariance
        # distance between fast features and slow centroid

        #fast feature: mean + covarance
        # fast_feature_subset = fast_projections[mask_]
        # fast_feature_subset_mean = fast_feature_subset[:,:3]
        # fast_feature_subset_covariance = fast_feature_subset[:,3:]
        # make sure the output is positive
        # fast_feature_subset_covariance[...,3:] =  F.elu(fast_feature_subset_covariance[...,3:]) + 1.0 + min_covariance
        ...
        # print(mean_1.shape)
        # print(covariance_1.shape)
        
        # print(distribution_2.shape)
        # print(mean_2.shape)
        # print(covariance_2.shape)
    ...
    def calculate_cross_distribution_distance(self, distribution_1, distribution_2):
        #
    ...
    def get_regularization_term(self, distribution):
        covariance = distribution[:,3:]
        min_covariance = 0.01
        covariance =  F.elu(covariance) + 1.0 + min_covariance
        regularization_term = torch.mean(torch.clamp(covariance[...,0] * covariance[...,1] * covariance[...,2], 1e-6, 1e+6))
        # regularization_term = regularization_term.mean()
    ...
    def calculate_instance_clustering_loss(self, labels_gt, instance_features, confidences, points_xyz=None):
        ...
        if self.instance_loss_mode == "slow_fast":
            ...
            # slow_centroids_mean = slow_centroids[:,:3]
            # slow_centroids_covariance = slow_centroids[:,3:]
            # min_covariance = 0.01
            # print(instance_map.shape)
            # slow_centroids_covariance[...,3:] =  F.elu(slow_centroids_covariance[...,3:]) + 1.0 + min_covariance
            ...
            for l in intersecting_labels:
                ...
                # slow feature: mean + covarance
                centroid_ = slow_centroids[slow_labels==l] # [1, d]
                # centroid_mean_ = centroid_[:,:3]
                # centroid_covariance_ = centroid_[:,3:]
                # make sure the output is positive
                # centroid_covariance[...,3:] =  F.elu(centroid_covariance[...,3:]) + 1.0 + min_covariance
                # distance between fast features and slow centroid
                ...
                ## distribution distance
                #
                # alpha_distribution = 4 * (centroid_covariance + fast_feature_subset_covariance)
                # alpha_distribution = torch.clamp(alpha_distribution,1e-6,1e+6)
                # distribution_dist = torch.sum((fast_feature_subset_mean- centroid_mean_)/(alpha_distribution), dim=-1)
                # distribution_dist = torch.exp(-1 * distribution_dist)
                
                # beta_distribtion = torch.sqrt(centroid_covariance)/torch.sqrt(fast_feature_subset_covariance) +  torch.sqrt(fast_feature_subset_covariance)/torch.sqrt(centroid_covariance)
                # beta_distribtion = torch.cumprod(beta_distribtion/2, dim=-1).clamp(1e-6, 1e+6)
                # beta_distribtion = 1.0/torch.sqrt(beta_distribtion)
                distribution_distance = self.calculate_distribution_distance(centroid_ , fast_feature_subset)
                # distribution_distance = self.calculate_distribution_distance(centroid_mean_, centroid_covariance_, fast_feature_subset_mean, fast_feature_subset_covariance)
                dist_sq = -1.0 * (distribution_distance *  confidences[mask_]).mean()
                loss += dist_sq
            ...
            similarity_matrix =  self.calculate_cross_distribution_distance(fast_projections[fast_mask], slow_projections[slow_mask])
            ...
            ############################### regularization for the covaraiance
            regularization_term = self.get_regularization_term(fast_projections[fast_mask])
            loss += regularization_term * 0.00
            #############################
    ...
    def train_dataloader(self):
        ...
        train_instance_set = get_inconsistent_single_dataset_two_frame(self.config)
...
def main(config):
    if config.template.dataset_class=="panopli":
        ...
        name = "MOS"
    elif config.template.dataset_class=="multi_frame_panopli":
        name = "multi_frame_panopli"
    elif config.template.dataset_class=="multi_frame_mos":
        name = "multi_frame_mos"
    elif config.template.dataset_class=="multi_frame_mos_debug":
        name = "multi_frame_mos_debug"
    
    
    scene_name = config.template.dataset_root.split("/")[-1]
    ## put the checkpoint in here
    ckpt = glob.glob(f"/research/d1/gds/rszhu22/Contrastive-Lift/runs/MOS_covariance_001/{scene_name}_res/checkpoints/epoch=7-*.ckpt")[0]

    
    config.template.resume = ckpt
    # print(scene_name)
    # print(config.template.dataset_root)
