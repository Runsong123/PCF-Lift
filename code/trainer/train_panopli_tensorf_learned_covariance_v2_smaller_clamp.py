...
def main(config):
    ...
    if len(glob.glob(f"/research/d1/gds/rszhu22/Contrastive-Lift/runs/moc_paper_rerun/*_{scene_name}_*/checkpoints/epoch=5-step=??????.ckpt"))==0:
        ckpt = sorted(glob.glob(f"/research/d1/gds/rszhu22/Contrastive-Lift/runs/moc_paper_rerun/*_{scene_name}_*/checkpoints/epoch=5-step=???????.ckpt"))[0]    
    else:
        ckpt = sorted(glob.glob(f"/research/d1/gds/rszhu22/Contrastive-Lift/runs/moc_paper_rerun/*_{scene_name}_*/checkpoints/epoch=5-step=??????.ckpt"))[0]
    checkpoint = torch.load(ckpt)
    
    new_mlp_weight = torch.randn_like(model.model.render_instance_mlp.mlp[6].weight)
    new_mlp_weight[:3,:] = checkpoint['state_dict']['model.render_instance_mlp.mlp.6.weight'][:3,:]
    new_mlp_weight[3:,:] = model.model.render_instance_mlp.mlp[6].weight[3:,:]
    checkpoint['state_dict']['model.render_instance_mlp.mlp.6.weight'] = new_mlp_weight

    new_mlp_weight = torch.randn_like(model.model.render_instance_mlp.mlp[6].bias)
    new_mlp_weight[:3] = checkpoint['state_dict']['model.render_instance_mlp.mlp.6.bias'][:3]
    new_mlp_weight[3:] = model.model.render_instance_mlp.mlp[6].bias[3:]
    checkpoint['state_dict']['model.render_instance_mlp.mlp.6.bias'] = new_mlp_weight

    new_mlp_weight = torch.randn_like(model.model.render_instance_mlp.slow_mlp[6].weight)
    new_mlp_weight[:3,:] = checkpoint['state_dict']['model.render_instance_mlp.slow_mlp.6.weight'][:3,:]
    new_mlp_weight[3:,:] = model.model.render_instance_mlp.slow_mlp[6].weight[3:,:]
    checkpoint['state_dict']['model.render_instance_mlp.slow_mlp.6.weight'] = new_mlp_weight


    new_mlp_weight = torch.randn_like(model.model.render_instance_mlp.slow_mlp[6].bias)
    new_mlp_weight[:3] = checkpoint['state_dict']['model.render_instance_mlp.slow_mlp.6.bias'][:3]
    new_mlp_weight[3:] = model.model.render_instance_mlp.slow_mlp[6].bias[3:]
    checkpoint['state_dict']['model.render_instance_mlp.slow_mlp.6.bias'] = new_mlp_weight


    
    exp_avg = torch.zeros_like(model.model.render_instance_mlp.mlp[6].weight)
    exp_avg[:3,:] = checkpoint['optimizer_states'][1]['state'][6]['exp_avg']
    checkpoint['optimizer_states'][1]['state'][6]['exp_avg'] = exp_avg

    exp_avg_sq = torch.zeros_like(model.model.render_instance_mlp.mlp[6].weight)
    exp_avg_sq[:3,:] = checkpoint['optimizer_states'][1]['state'][6]['exp_avg_sq']
    checkpoint['optimizer_states'][1]['state'][6]['exp_avg_sq'] = exp_avg_sq

    ##########
    exp_avg = torch.zeros_like(model.model.render_instance_mlp.mlp[6].bias)
    exp_avg[:3] = checkpoint['optimizer_states'][1]['state'][7]['exp_avg']
    checkpoint['optimizer_states'][1]['state'][7]['exp_avg'] = exp_avg

    exp_avg_sq = torch.zeros_like(model.model.render_instance_mlp.mlp[6].bias)
    exp_avg_sq[:3] = checkpoint['optimizer_states'][1]['state'][7]['exp_avg_sq']
    checkpoint['optimizer_states'][1]['state'][7]['exp_avg_sq'] = exp_avg_sq
    
    
    ckpt_path_output_dir = f"/research/d1/gds/rszhu22/Contrastive-Lift/runs/moc_paper_rerun/simplify_checkpoint_new_for_smaller_clamp/{scene_name}/"
    os.makedirs(ckpt_path_output_dir, exist_ok=True)
    new_ckpt = ckpt.split("checkpoints/")[1] 
    torch.save(checkpoint, f"{ckpt_path_output_dir}/{new_ckpt}")

    config.template.resume = f"{ckpt_path_output_dir}/{new_ckpt}"
