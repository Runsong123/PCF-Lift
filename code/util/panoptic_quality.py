...
def panoptic_quality_debug(
    ...
    iou_sum, true_positives, false_positives, false_negatives, mapping_dict = _panoptic_quality_update_debug(
    ...
    return results["all"]["pq"], results["all"]["sq"], results["all"]["rq"], mapping_dict


def _panoptic_quality_update_debug(
    ...
    for (pred_color, target_color), intersection in intersection_areas.items():
        ...
        if iou > 0.5:
            ...
            print("pred_color", pred_color)
            target_segment_matched.add(target_color)
            print("target_color", target_color)
            iou_sum[continuous_id] += iou
            true_positives[continuous_id] += 1
            ### mapping from pred >> GT
            corresponding_dict[pred_color[1]]=target_color[1]
    ...
    return iou_sum, true_positives, false_positives, false_negatives,corresponding_dict
