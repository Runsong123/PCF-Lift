...
def ae_loss(features, instance_labels, sigma=1.0):
    ...
    pull_dis = torch.norm(features - centroids[inverse_indices], dim=-1)
    pull_loss = torch.exp(pull_dis * sigma).mean()
    ...
    distances = torch.norm(centroids.unsqueeze(1) - centroids.unsqueeze(0), dim=-1) # (num_instances, num_instances)
    ...
    distances_nondiag = torch.relu(0.5 - distances_nondiag)
